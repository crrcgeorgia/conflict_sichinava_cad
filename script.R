...
library(ggeffects)
library(MASS)
setwd("D:\\Dropbox\\My projects\\CRRC\\Caucasus Barometer\\2019\\analysis\\cad\\conflicts")


historical <- read_excel("historical_data.xlsx", sheet=1, "B3:N8")%>%
  gather(variable, value, -`Abkhazia and South Ossetia should be…`)%>%
  set_colnames(c("item", "wave", "value"))%>%
  mutate(item=factor(item, levels=c(1:5),
                     labels=c("Ordinary regions of Georgia, without any autonomy", "Autonomies of [the] Georgian state", "Autonomies of Russian Federation", "Independent [states]", "No opinion")),
         wave=factor(wave, levels=c(1:12),
                     labels=c("May-03", "Feb-04", "Jun-04", "Oct-04", "Jun-05", "Oct-05", "Apr-06", "Feb-07", "Sep-07", "Feb-08", "Sep-08", "Feb-09")))

ndi <- read_excel("historical_data.xlsx", sheet=2, "A5:AA28")%>%
  gather(variable, value, -`issue`)%>%
  set_colnames(c("item", "wave", "value"))%>%
  mutate(item=factor(item),
         wave=factor(wave, levels=c(1:26),
                     labels=c("May-09", "Dec-09", "Apr-10", "Jul-10", "Mar-11", "Sep-11", "Feb-12", "Jun-12", "Aug-12", "Nov-12", "Mar-13", "Jun-13", "Sep-13", "Nov-13", "Apr-14", "Aug-14", "Apr-15", "Nov-15", "Mar-16", "Jun-16", "Nov-16", "June-17", "Dec-17", "June-18", "Dec-18", "Jul-19")))
